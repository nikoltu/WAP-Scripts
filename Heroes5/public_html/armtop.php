...
$limit = $page * 20;
$llimit = $limit + 20;
$cnt=mysql_query("SELECT COUNT(username) AS num FROM army");
$count=($cnt) ? mysql_result($cnt, 0, 'num') : 0;
echo"<small><b><u>Armiju TOP</u></b></small><br/>$line<br/><small>J&#363;s u&#382;imate <b>$pl</b> viet&#261;.</small><br/>$line</p><p align=\"left\">";
$on = mysql_query("SELECT username, quantity,unit FROM army ORDER by quantity DESC LIMIT $limit, 20");
$top = 0;
...
if ($pages > $page + 1) {
   $pager = $page + 1;
   echo"<small><a href=\"index.php?action=armtop&amp;id=$id&amp;page=$pager\">$next</a></small><br/>";
}
if ($page > 0) {
   $pager = $page - 1;
