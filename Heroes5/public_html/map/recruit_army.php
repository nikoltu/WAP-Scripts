...
if ($object[id] == "") {
   $queries++;
   mysql_query("INSERT INTO objects(location, object, time, info) VALUES ('$pilis', '$build', '$time', '$object[info]')");
}
elseif ($object[time] + $refresh_time < $time) {
   $queries++;
   mysql_query("UPDATE objects SET time='$time', info='$object[info]' WHERE location='$pilis' and object='$build' LIMIT 1");
}
...
if ($n == "") {
   ...
   if ($total_units < 5) {
      ...
      echo"<small><u>Turite $punit_name: $have</u></small><br/><input format=\"*N\" name=\"h$time\" maxlength=\"6\" size=\"6\" value=\"$max\"/><br/><small><anchor>[+Pirkti+]<go href=\"index.php?action=castle&amp;id=$id&amp;door=upgrade&amp;build=$build&amp;pilis=$pilis&amp;n=buy&amp;nn=$time\" method=\"post\"><postfield name=\"$time\" value=\"$(h$time)\"/></go></anchor></small><br/>";
      if ($non !== "none") {
         echo"$line<br/>";
      }
   }
   if ($non !== "none") {
      ...
      echo"<small><u>Turite $punit_name: $army_quantity[$non]</u></small><br/><input format=\"*N\" name=\"h$ttime\" maxlength=\"6\" size=\"6\" value=\"$max\"/><br/><small><anchor>[+Tobulinti+]<go href=\"index.php?action=castle&amp;id=$id&amp;door=upgrade&amp;build=$build&amp;pilis=$pilis&amp;n=upgrade&amp;nn=$time\" method=\"post\"><postfield name=\"$ttime\" value=\"$(h$ttime)\"/></go></anchor></small><br/>";
   }
...
mysql_query("UPDATE objects SET info=info-$q where location='$pilis' and object='$build'");
