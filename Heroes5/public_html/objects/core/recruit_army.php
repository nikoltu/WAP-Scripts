...
if ((($object[id] == "")) or (($object[time] + $refresh_time < $time))) {
   $object[info] = 10 * mt_rand($basic + floor($count * $i1), $basic + floor($count * $i1));
}
...
if ($n == "") {
   ...
   if (($uunit_info[shoot] == "1") or ($uunit_info[spec] !== "") or ($uunit_info[spec2] !== "") or ($uunit_info[spec3] !== "") or ($uunit_info[spec4] !== "") or ($uunit_info[spec5] !== "")) {
      ...
      if ($uunit_info[spec3] !== "") {
         $special = $special_name[$uunit_info[spec3]];
         echo"<small><i>$special</i></small><br/>";
      }
      if ($uunit_info[spec4] !== "") {
         $special = $special_name[$uunit_info[spec4]];
         echo"<small><i>$special</i></small><br/>";
      }
      if ($uunit_info[spec5] !== "") {
         $special = $special_name[$uunit_info[spec5]];
         echo"<small><i>$special</i></small><br/>";
      }
   }
   if ($uunit_info[cost2] !== "") {
   ...
   if ($army_unit[4] == "$unit") { $have = $army_quantity[4]; }
   if ($army_unit[5] == "$unit") { $have = $army_quantity[5]; }

   if ($army_unit[6] == "$unit") { $have = $army_quantity[6]; }
   if ($army_unit[7] == "$unit") { $have = $army_quantity[7]; }
   $non = "none";
   if ($uunit_info[nonupg] !== "") {
      ...
      if ($army_unit[4] == "$uunit_info[nonupg]") { $non = 4; }
      if ($army_unit[5] == "$uunit_info[nonupg]") { $non = 5; }
      if ($army_unit[6] == "$uunit_info[nonupg]") { $non = 6; }
      if ($army_unit[7] == "$uunit_info[nonupg]") { $non = 7; }
   }
   if (($have == "0") and ($total_units == "$toar")) {
      echo"<small>Negalite tur&#279;ti daugiau nei $toar r&#363;&#353;i&#371; kari&#371;.</small><br/>";
   }
   if ($total_units < $toar+1) {
      if ($uunit_info[cost2] !== "") {
   ...
   if (($total_units < $toar) or ($non !== "none")) {
      echo"$line<br/>";
...
if ($n == "upgrade") {
if($i=="land_of_darkness"){
echo"<small>Tamsybi&#371; kra&#353;te negalima tobulinti kari&#371;.Tik pirkti</small></p></card></wml>";exit;mysql_close($db);}
   $nn = trim(addslashes(htmlspecialchars($_GET['nn'])));
   $q = trim(addslashes(htmlspecialchars($_POST["$nn"])));
...
elseif($army_mag[$non]=="hipnoze"){
echo"<small>Negalima tobulinti uzhipnotizuot&#371; kari&#371;</small>";}
elseif($army_mag[$non]=="reanim"){
echo"<small>Negalima tobulinti reanimuot&#371; kari&#371;</small>";}
   elseif (($q > 0) and ($non < $toar)) {
      ...
      elseif ($non !== "none") {
         if (($total_units == "$toar") and ($q < $army_quantity[$non])) {
         ...
         elseif ($q >= $army_quantity[$non]) {
            ...
            if (($army_unit[0] == $unit) or ($army_unit[1] == $unit) or ($army_unit[2] == $unit) or ($army_unit[3] == $unit) or ($army_unit[4] == $unit) or ($army_unit[5] == $unit) or ($army_unit[6] == $unit) or ($army_unit[7] == $unit)) {
               $queries++;
               $queries++;
               ...
               $queries++;
$hpa=mysql_fetch_array(mysql_query("SELECT * FROM artifacts where user='$user[username]' and det='1' and name='ring_of_vitality'"));

if($hpa){
$health=$health+1;}
$hpaa=mysql_fetch_array(mysql_query("SELECT * FROM artifacts where user='$user[username]' and det='1' and name='vial_of_lifeblood'"));
if($hpaa){
$health=$health+2;}               mysql_query("UPDATE army SET unit='$unit', attack=attack+$attack, defense=defense+$defense, health=health+$health, min_damage=min_damage+$min, max_damage=max_damage+$max WHERE username='$name' and unit='$uunit_info[nonupg]' LIMIT 1");
            }
            if (strlen($res1[1]) > 0) {
         ...
         elseif ($q < $army_quantity[$non]) {
            ...
            if (($army_unit[0] == $unit) or ($army_unit[1] == $unit) or ($army_unit[2] == $unit) or ($army_unit[3] == $unit) or ($army_unit[4] == $unit) or ($army_unit[5] == $unit) or ($army_unit[6] == $unit) or ($army_unit[7] == $unit)) {
               ...
               $name = strtolower($user[username]);
               $queries++;
               $hpa2=mysql_fetch_array(mysql_query("SELECT * FROM artifacts where user='$user[username]' and det='1' and name='ring_of_vitality'"));

if($hpa2){
$unit_info[health]=$unit_info[health]+1;}
$hpaa2=mysql_fetch_array(mysql_query("SELECT * FROM artifacts where user='$user[username]' and det='1' and name='vial_of_lifeblood'"));
if($hpaa2){
$unit_info[health]=$unit_info[health]+2;}               mysql_query("INSERT INTO army(username, unit, quantity, attack, defense, min_damage, max_damage, health) VALUES ('$name','$unit','$q','$unit_info[attack]','$unit_info[defense]','$unit_info[min]','$unit_info[max]','$unit_info[health]')");
               $queries++;
               mysql_query("UPDATE army SET quantity=quantity-$q WHERE username='$name' and unit='$uunit_info[nonupg]' LIMIT 1");
            }
            if (strlen($res1[1]) > 0) {
...
if ($n == "buy") {
   ...
   if (($q > 0) and ($have == "0") and ($total_units == "$toar")) {
      echo"<small>Negalite tur&#279;ti daugiau nei $toar r&#363;&#353;i&#371; kari&#371;.</small><br/>";
   ...
   elseif (($q > 0) and ($total_units < $toar) and ($have == "0")) {
      if ($unit_info[cost2] !== "") {
         $res = explode("-", $unit_info[cost2]);
      }
      if ($unit_info[cost2] !== "") {
