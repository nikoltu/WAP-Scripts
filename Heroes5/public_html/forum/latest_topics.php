...
$hot_topics = 250;
$login = $user[llogin];
echo"<small><b>Naujausios Temos</b></small><br/>$line</p><p align=\"left\">";
$limit = $page * $show_topics;
if ($lvl > 2) {
	$topics = mysql_query("SELECT id, topic, posts, closed, time, bold, forum FROM topics WHERE hidden='0' ORDER by created DESC LIMIT $limit, $show_topics");
}
else {
	$topics = mysql_query("SELECT id, topic, posts, closed, time, bold, forum FROM topics WHERE hidden='0' and forum>1 ORDER by created DESC LIMIT $limit, $show_topics");
}
...
if ((($page * $show_topics) + $show_topics) < $ttopics) {
	$pager = $page + 1;
	echo"<small><a href=\"forum.php?action=newtopics&amp;id=$id&amp;page=$pager\">$next</a></small><br/>";
}
if ($page > 0) {
	$pager = $page - 1;
	echo"<small><a href=\"forum.php?action=newtopics&amp;id=$id&amp;page=$pager\">$back</a></small><br/>";
}
$page++;
echo"<small>Puslapis: $page/$pages</small><br/>$line<br/><small>Pastaba: parodoma 20 naujausi&#371; tem&#371;.</small><br/>$line<br/><small><a href=\"forum.php?id=$id\">$back_forums</a></small>";
?>