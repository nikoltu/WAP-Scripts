...
$queries++;
$battle = mysql_fetch_array(mysql_query("SELECT * FROM cbattle WHERE id='$m' LIMIT 1"));
if ($battle[active] == "1") {
   $queries++;
   $event = mysql_fetch_array(mysql_query("SELECT * FROM castle WHERE id='$m' LIMIT 1"));
   if ($event[id] > 0) {
      $queries++;
      mysql_query("DELETE FROM cbattle WHERE id='$m' LIMIT 1");
   }
...
include_once("skils/luck.php");
      ...
      $queries++;
      mysql_query("INSERT INTO cbattle(id, health, heroe, round, turn, unit, expierence, resource, q_unit, q_res, artifact, time) VALUES ('$m', '$unit_health', '$battle[heroe]', '$battle[round]', '$battle[turn]', '$battle[unit]', '$battle[expierence]', '$battle[resource]', '$battle[q_unit]', '$battle[q_res]', '$battle[artifact]','$time')");
   }
}
...
include_once("skils/luck.php");
      ...
      $queries++;
      mysql_query("UPDATE cbattle SET heroe='$name', round='$battle[round]', turn='$battle[turn]', q_unit='$event[q_unit]', health='$battle[health]', time='$time' WHERE id='$m' LIMIT 1");
      $queries++;
      mysql_query("UPDATE army SET hp='0' WHERE username='$battle[heroe]'");
      }
   else {
...
$dmg2=($uunit_health*0.1)*$battle[q_unit];}
      if ($dmg >= $battle[health]) {
         ...
         $queries++;
         mysql_query("UPDATE cbattle SET q_unit=q_unit-$kill, health='$battle[health]', expierence='$exp' WHERE id='$m' LIMIT 1");
      }
      else {
         $battle[health] = $battle[health] - $dmg;
         $queries++;
         mysql_query("UPDATE cbattle SET health='$battle[health]' WHERE id='$m' LIMIT 1");
      }
...
include_once("include/nt2.php");}}
   ...
   if (($battle[q_unit] > 0) and (($army_quantity[0] > 0) or ($army_quantity[1] > 0) or ($army_quantity[2] > 0) or ($army_quantity[3] > 0))) {
      echo"<small><anchor>&#187;&#187;&#187;<go method=\"post\" href=\"index.php?action=castle&amp;id=$id&amp;pilis=$pilis&amp;door=atk&amp;time=".time()."\"><postfield name=\"event\" value=\"$m\"/></go></anchor></small><br/>";
   }
}
elseif ($turn[0] == "a") {
   include("names/units.php");
...
echo"<small>Nuodu poveikis $dmg2.</small><br/>";}
   ...
   if (($turn[1] < $total_units) and ($battle[q_unit] > 0)) {
      ...
      echo"<small><b>Esate $army_quantity[$whoo] $unit_name</b></small><br/><small>Pasirinkite k&#261; atakuoti:</small><br/><small><anchor>$battle[q_unit] $uunit_name<go method=\"post\" href=\"index.php?action=castle&amp;id=$id&amp;pilis=$pilis&amp;door=atk&amp;time=".time()."\"><postfield name=\"event\" value=\"$m\"/></go></anchor></small><br/>$line<br/><small><anchor>Praleisti &#279;jim&#261;<go method=\"post\" href=\"index.php?action=castle&amp;id=$id&amp;pilis=$pilis&amp;door=atk&amp;time=no\"><postfield name=\"event\" value=\"$m\"/></go></anchor></small><br/>";
...
method=\"post\" href=\"index.php?action=castle&amp;id=$id&amp;pilis=$pilis&amp;door=atk&amp;magi=$name&amp;time=".time()."\"><postfield name=\"event\" value=\"$m\"/></go></anchor></small><br/>$line<br/>";}   }
   else {
      if (($battle[q_unit] > 0) and (($army_quantity[0] > 0) or ($army_quantity[1] > 0) or ($army_quantity[2] > 0) or ($army_quantity[3] > 0))) {
      echo"<small><anchor>&#187;&#187;&#187;<go method=\"post\" href=\"index.php?action=castle&amp;id=$id&amp;pilis=$pilis&amp;door=atk&amp;time=".time()."\"><postfield name=\"event\" value=\"$m\"/></go></anchor></small><br/>";
      }
...
$gtime=time()+$btime;
   ...
   $queries++;
   mysql_query("UPDATE cbattle SET q_unit='$event[q_unit]', active='1' WHERE id='$m' LIMIT 1");
   $queries++;
   mysql_query("DELETE FROM castle WHERE id='$m' LIMIT 1");
@unlink("spec/stoning/$battle[heroe].tx");
@unlink("spec/blind/$battle[heroe].tx");
...
$queries++;
mysql_query("UPDATE cbattle SET round='$battle[round]', turn='$turn[0]|$turn[1]' WHERE id='$m' LIMIT 1");
?>